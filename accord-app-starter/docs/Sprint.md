...
- [x] Focused Review & Send redesign: multiple representative review packages, exact-paperwork field records, Needs Attention first, Full Paperwork Review in document order, itemized addendum wording, complete transcript/source links, mock paperwork preview, and signature send blocked by unresolved required items.
- [x] Review & Send interaction refinement: mutually exclusive package status, explicit item approval, red-first sorting, collapsible major sections, complete transcript source-and-return navigation, clarified non-transcript provenance, preview highlights, and stable document anchors.
