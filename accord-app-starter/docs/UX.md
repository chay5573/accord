...
Review & Send should feel like exact paperwork review, not a dashboard or AI summary. It must show complete package names, multiple review packages when present, Needs Attention first, All Fields Review in document/section/field order, exact generated values, exact checkbox/radio selections, itemized addendum provisions, and an editable Paperwork section. Needs Attention, All Fields Review, and Paperwork reference the same underlying records so edits update all surfaces immediately. Source links open the complete transcript or applicable source detail and highlight the exact supporting content.
...
- Every unresolved review item has an explicit completion action. Missing or edited values use **Save & Approve**; unchanged generated values use **Approve**. Typing changes only temporary editor state: the row must retain its place and persisted review status until approval. Approval then updates Needs Attention, All Fields Review, Paperwork, package counts, and send readiness.
...
- Needs Attention, All Fields Review, Paperwork, Transcript, and Other Sources use accessible progressive disclosure. Needs Attention opens by default when work remains; the complete transcript remains collapsed until needed. Package metadata is a compact summary row, not a competing dashboard of metric cards.
...
- Paperwork is a first-class document viewer with document and page navigation. It supports the same editable text, checkbox, radio, selection, and addendum records used by All Fields Review. Red marks unresolved required values, amber marks values awaiting approval, pale yellow marks user edits, and approved values have no highlight.
